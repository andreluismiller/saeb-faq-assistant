...
class RAGSystem:
    """Sistema RAG integrando Qdrant, FastEmbed e LLM via Groq."""

    # Campo do payload no Qdrant usado para filtrar por programa/pesquisa (ex: "SAEB", "ENADE"...)
    PROGRAM_FIELD = "survey"

    # Instruções fixas do sistema (persona + regras). Mantidas separadas do prompt final
    # para que possam ser persistidas isoladamente na coluna "instructions" do Postgres.
    INSTRUCTIONS = """
...
""".strip()
    ...
    def list_programs(self):
        """
        Retorna a lista de valores distintos do campo PROGRAM_FIELD ("survey") presentes
        na coleção, para popular o seletor de programas na UI.
        """
        programs = set()
        next_offset = None
        try:
            while True:
                ...
                for point in points:
                    value = point.payload.get(self.PROGRAM_FIELD)
                    if value:
                        programs.add(value)
    ...
    def _build_program_filter(self, program: Optional[str]) -> Optional[Filter]:
        if not program or program.strip().lower() == "none":
            return None
        return Filter(
            must=[FieldCondition(key=self.PROGRAM_FIELD, match=MatchValue(value=program))]
        )

    def hybrid_search(self, query: str, limit: int = 5, program: Optional[str] = None):
        """
        Executa busca híbrida (Semântica + Lexical) com Reciprocal Rank Fusion (RRF).
        Se "program" for informado (ex.: "SAEB"), restringe a busca aos documentos
        cujo payload["survey"] corresponda ao valor informado.
        """
        ...
        query_filter = self._build_program_filter(program)
    ...
    def build_context(self, search_results: list) -> str:
        """Monta o bloco de contexto a partir dos resultados da busca."""
        context = ""
    ...
    def build_prompt(self, user_query: str, search_results: list) -> str:
        """Constrói o prompt final (instruções + contexto + pergunta) enviado ao LLM."""
...
""".strip()
        ...
        return prompt_template.format(
            instructions=self.INSTRUCTIONS, context=context, question=user_query
        )
    ...
    def execute_rag_pipeline(self, user_query: str, program: Optional[str] = None):
        """
        Orquestra o workflow completo: Validação -> Busca -> Prompt -> LLM -> Métricas.
        "program" (opcional) filtra a busca por um programa/pesquisa específico (ex.: "SAEB").
        """
        ...
        search_results = self.hybrid_search(user_query, program=program)
